            ...
            >
              ...
              <div class="flex flex-col gap-2">
                <template v-if="displayLines1.length > 0">
                  <div v-for="(group, idx) in displayLines1" :key="idx" class="flex flex-col group/item">
                    <div class="flex items-center gap-3 transition-all duration-300" :class="{ 'opacity-30 grayscale': group[0].isPastOrCurrent }">
                      <div class="relative flex flex-col items-center">
                         <div class="w-2.5 h-2.5 rounded-full shadow-sm transition-colors" :class="[group[0].isPastOrCurrent ? 'bg-gray-400' : 'bg-indigo-500 ring-4 ring-indigo-100']"></div>
                         <div v-if="idx < displayLines1.length - 1" class="absolute top-2.5 w-0.5 h-6 transition-colors" :class="[group[0].isPastOrCurrent ? 'bg-gray-200' : 'bg-indigo-100']"></div>
                      </div>
                      <div class="flex flex-wrap gap-x-3 gap-y-1 items-center">
                        <template v-for="(line, lIdx) in group" :key="line.approver_user_id">
                          <span class="text-[13px] text-gray-800 transition-all flex items-center" :class="{ 'font-bold': !line.isPastOrCurrent }">
                            <span class="text-gray-400 mr-1 text-[11px] font-medium">{{ line.approver_role }}</span> {{ line.approver_user_name }}
                          </span>
                          <span v-if="lIdx < group.length - 1" class="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-sm font-bold">OR</span>
                        </template>
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <div class="flex flex-col gap-2">
                <template v-if="displayLines2.length > 0">
                  <div v-for="(group, idx) in displayLines2" :key="idx" class="flex flex-col group/item">
                    <div class="flex items-center gap-3 transition-all duration-300" :class="{ 'opacity-30 grayscale': group[0].isPastOrCurrent }">
                      <div class="relative flex flex-col items-center">
                         <div class="w-2.5 h-2.5 rounded-full shadow-sm transition-colors" :class="[group[0].isPastOrCurrent ? 'bg-gray-400' : 'bg-purple-500 ring-4 ring-purple-100']"></div>
                         <div v-if="idx < displayLines2.length - 1" class="absolute top-2.5 w-0.5 h-6 transition-colors" :class="[group[0].isPastOrCurrent ? 'bg-gray-200' : 'bg-purple-100']"></div>
                      </div>
                      <div class="flex flex-wrap gap-x-3 gap-y-1 items-center">
                        <template v-for="(line, lIdx) in group" :key="line.approver_user_id">
                          <span class="text-[13px] text-gray-800 transition-all flex items-center" :class="{ 'font-bold': !line.isPastOrCurrent }">
    ...
    <div class="flex justify-between">
      ...
      <div class="flex gap-3">
        <button
          ...
          class="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-purple-700 text-white px-6 py-3 rounded-lg shadow-md transition disabled:opacity-70 disabled:cursor-not-allowed disabled:pointer-events-none text-sm font-medium"
        >
...
const displayLines1 = computed(() => {
  ...
  return Object.values(groups).map(group => {
    // 본인 순번인 경우(또는 찾지 못한 경우 첫 단계) 본인만 필터링
    if (group.length > 0) {
...
const sendApprovalRequest = async () => {
  ...
  try {
    ...
    const data = {
      ...
      accountInfo: props.accountInfo,
      requesterPhone: props.requesterPhone,
      updateDeptAccountInfo: props.updateDeptAccountInfo,
      items: normalizeItems(props.items),
